...
- **`ProtectedRoute`**: Listens to the `AuthContext`. If no mock token is found in `localStorage`, it redirects to the Sign-In page. Used for the Dashboard, Analytics, and Upload screens.
- **`PublicRoute`**: The inverse of ProtectedRoute. It prevents authenticated users from accidentally returning to the Sign-In or Sign-Up pages by redirecting them back to their secure workspace.
